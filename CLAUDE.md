...
- **Status:** **Milestone 3 complete.** The immune system is live. The **Diagnostician (0x00E5)** taps the *whole* bus (it overrides the SIL listener to observe the raw stream rather than filter it — the one agent whose job is to watch everything), reconstructs every in-flight cascade by its **correlation/choreography id** (the `turn_id` when present — recovered from TRIGGER parameters, from the asset chain via the `AssetStore`, or from the cascade an agent is already part of — else the originating TRIGGER's id, so even the M0 handshake correlates), and keeps a timed **`SDR_ChoreographyLog`** (the CHOREOGRAPHY_LOG) per cascade. It **detects loops** (hop-count / repeated-signature runaways) and **stalls** (a cascade that ACKs then goes quiet — the hung-agent case M2 surfaced, found via `sweep_timeouts()` with an injectable clock), **trips a `CircuitBreaker`** that the bus consults before delivery (targeted by source UID, plus a global kill-switch) to contain a runaway while the rest of the Family keeps working, runs a **recovery cascade** (abort cleanly + release the breaker where safe), and emits an **`SDR_AnomalyReport`** + the finalized CHOREOGRAPHY_LOG to the Glass Box. It is a **passive observer in the healthy path** — zero added bus traffic on a normal turn, so it never perturbs the flow it measures. Built on M0 (nervous system) + M1 (3-agent cascade) + M2 (the conscience). Proven by `python main.py --diagnostics` and **50 passing tests**. Working branch: `claude/aletheia-milestone-3-9phw45`.
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev]"`, then `python main.py` (Q&A console — set `ANTHROPIC_API_KEY` for live Claude, else offline/extractive mode; commands `/log`, `/health`, `/quit`), `python main.py --diagnostics` (M3 immune-system demo: induce a loop + a hung agent, watch detect → contain → heal), `python main.py --handshake` (M0 demo), or `python -m pytest` (tests). A SessionStart hook (`.claude/`) auto-installs deps in web sessions.
- **Next action:** Milestone 4 in `ROADMAP.md` — the **knowledge-graph Archivist (v2)**: spaCy (`en_core_web_lg`) deterministic parsing → entities + relations → a `GraphStore` interface, with hybrid (vector + graph) retrieval and `SDR_Knowledge_Graph_Element` / `SDR_Fact_Assertion` provenance. (This is also what fixes the M1 limitation where simple TF-IDF retrieval misses relational questions like "What does the Philosopher enforce?")
