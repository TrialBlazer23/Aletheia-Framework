...
- **Status:** **Milestone 2 complete.** The conscience is live: **Nexus-Mind (0x0001) → Archivist (0x00A1) → Narrator (0x00B2) → Philosopher (0x00C3) → user.** The four Prime Directives are encoded in `aletheia/safety/prime_directives.yaml`; a rule-based `RuleBasedValidator` (regex/keyword, redacted evidence) sits on `DRAFT_READY` with **veto power**, emits `APPROVED`/`REJECTED`, and logs every verdict as an `SDR_Ethical_Analysis_Report` (cited directive + reasoning + escalation flag). A turn now returns a `QAResult` (answer **+** verdict). The Narrator degrades gracefully if the live model errors (no credits / bad key / network) — falls back to a grounded extract instead of hanging the cascade. Built on M0 (nervous system) + M1 (3-agent cascade). Proven by `python main.py` and **36 passing tests**. Working branch: `claude/framework-roadmap-setup-94bf37`.
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev]"`, then `python main.py` (Q&A console — set `ANTHROPIC_API_KEY` for live Claude, else offline/extractive mode), `python main.py --handshake` (M0 demo), or `python -m pytest` (tests). A SessionStart hook (`.claude/`) auto-installs deps in web sessions.
- **Next action:** Milestone 3 in `ROADMAP.md` — the **Diagnostician (0x00E5)**: monitor the bus, track in-flight cascades by correlation id, detect loops/timeouts (e.g. the stalled-cascade case M2 surfaced), trip a circuit breaker, and produce `CHOREOGRAPHY_LOG` telemetry.
