...
- **Status:** **Milestone 1 complete.** The first living cascade runs: **Nexus-Mind (0x0001) → Archivist (0x00A1) → Narrator (0x00B2)** answering questions over Aletheia's own docs. Built on the Milestone 0 nervous system (protocol/bus/SIL/Cascade Log). New layers: `LLMProvider` interface with a **Claude** default (`aletheia/llm/`, lazy-imported `anthropic`, model `claude-opus-4-8`) + an offline extractive fallback so it runs with no API key; `VectorStore` interface with a dependency-free TF-IDF retriever and an `AssetStore` (NSAP-0003 pointers) (`aletheia/memory/`); SDR Tier-1 primitives + cascade assets (`aletheia/sdr/`); the three agents (`aletheia/agents/`); and the `QASystem` + console (`aletheia/app/`). Proven by `python main.py` (Q&A console over 395 ingested passages, full domino in the Cascade Log) and **25 passing tests**. Working branch: `claude/framework-roadmap-setup-94bf37`.
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev]"`, then `python main.py` (Q&A console — set `ANTHROPIC_API_KEY` for live Claude answers, else offline extractive mode), `python main.py --handshake` (M0 demo), or `python -m pytest` (tests).
- **Next action:** Milestone 2 in `ROADMAP.md` — the **Philosopher (0x00C3)**: encode the four Prime Directives as `prime_directives.yaml`, add a rule-based validator that sits on `DRAFT_READY` with veto power (emits `APPROVED`/`REJECTED`), logs every verdict as an `SDR_Ethical_Analysis_Report`, and inserts into the cascade before the user.
