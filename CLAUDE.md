...
- **Status:** **Milestone 4 complete.** The Archivist now has **grounded, neuro-symbolic memory** — the anti-hallucination core. On ingest it does two things with each doc: indexes it for vectors **and** runs a **deterministic spaCy dependency parse** (`SpacyExtractor`, behind a `KnowledgeExtractor` interface; prefers `en_core_web_lg`, falls back to `md`/`sm`, then a dependency-free `RuleBasedExtractor`) to pull typed entities + subject–predicate–object **facts** into a knowledge graph (`NetworkXGraphStore`, behind the `GraphStore` interface). Extraction handles plain SVO, copulas, **and participial modifiers** ("the Philosopher *enforcing* the Prime Directives"), and filters markdown/code/pronoun junk. Retrieval is **hybrid**: vector passages **+** graph traversal, where `find_facts()` ranks facts by relation-match (predicate ×2) and entity-match (×1) so a *relational* question lands on the relation it asked about. Every fact carries its **`Data_Source` + the evidence sentence** (`SDR_Fact_Assertion` / `SDR_Knowledge_Graph_Element`), so answers are traceable — even offline, the Narrator builds a cited relational answer straight from the graph. **Done-when proven:** "What does the Philosopher enforce?" → *"Philosopher safety kernel enforce Prime Directives (source: README.md)"*, via graph traversal, live and offline. Graph degrades gracefully (no networkx/spaCy → vector-only). Built on M0–M3. Proven by `python main.py --knowledge` and **67 passing tests**. Working branch: `claude/aletheia-milestone-4` (stacked on the M3 branch until it merges).
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev,graph]"` (the `graph` extra pulls spaCy + the `en_core_web_sm` model + networkx), then `python main.py` (Q&A console — set `ANTHROPIC_API_KEY` for live Claude, else offline/extractive mode; commands `/log`, `/health`, `/graph`, `/quit`), `python main.py --knowledge` (M4 knowledge-graph demo: relational questions answered by graph traversal, each fact cited), `python main.py --diagnostics` (M3 immune-system demo), `python main.py --handshake` (M0 demo), or `python -m pytest` (tests). A SessionStart hook (`.claude/`) auto-installs deps + the spaCy model in web sessions.
- **Next action:** Milestone 5 in `ROADMAP.md` — the **Resonance Cycle + Human Gavel** (supervised self-improvement): performance analytics over the `CHOREOGRAPHY_LOG`s (Efficiency/Fidelity/Coordination/Alignment), the four-phase Resonance Engine (Dissonance Detection → Harmonic Analysis → Proposal Generation → Integration), with **all** the safety gates — Philosopher sandbox simulation, rate-limiting, snapshot+rollback, and the Human Gavel (nothing applies without human approval). Behavior lives in a versioned `operational_policy.json`.
