...
- **Status:** **Milestone 5 complete.** The system now **learns from its own logged failures, supervised** — the Resonance Cycle (RFC-001 / NSAP-ARCH-003), with the **Human Gavel**. Behaviour lives in a versioned `OperationalPolicy` (`PolicyStore`, snapshot+rollback, optional `operational_policy.json`); the Archivist reads it at retrieval time (so a change — or rollback — takes effect with no re-ingest). The `ResonanceEngine` runs the four phases: **(I) Dissonance Detection** — `PerformanceAnalyzer` computes the four RFC-001 indices (Efficiency/Fidelity/Coordination/Alignment, weights 0.20/0.30/0.20/0.30) → "System Harmony"; a negative user `Feedback` collapses fidelity and flags dissonance. **(II) Harmonic Analysis** — trace the turn back to the offending source → `SDR_RootCauseAnalysis` (the FAILURE_REPORT). **(III) Proposal Generation** — a `SDR_PolicyModificationProposal`, **simulated in an isolated `ResonanceSandbox`** (read-only probe of the Archivist under a candidate policy), then **verified by the Philosopher** as Runtime Safety Kernel against the Prime Directives — VERIFIED only if the fix works *and* causes no collateral suppression of legitimate sources (else REJECTED, citing *Sanctity of Information Flow*). **(IV) Integration** — **rate-limited**, **snapshotted**, and applied **only after the Human Gavel** approves; **auto-rollback** if harmony falls after deploy. Every phase is emitted to the Glass Box. **Done-when proven:** replay the RFC-001 satire scenario — a `satire-news.com` "fact" is used, the user flags it, the cycle detects → analyses → proposes → sandbox-verifies → (human approves) → distrusts the source (policy v2) → the fact is gone; **rollback** restores it. Default gavel **denies** (nothing self-modifies without an explicit yes — the owner's "Human Gavel forever"). Built on M0–M4. Proven by `python main.py --resonance` and **90 passing tests**. Working branch: `claude/aletheia-milestone-5`.
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev,graph]"` (the `graph` extra pulls spaCy + the `en_core_web_sm` model + networkx), then `python main.py` (Q&A console — set `ANTHROPIC_API_KEY` for live Claude, else offline/extractive mode; commands `/log`, `/health`, `/graph`, `/feedback`, `/policy`, `/quit` — `/feedback` flags the last answer and engages the Resonance Cycle with a real terminal Human Gavel), `python main.py --resonance` (M5 self-healing demo), `python main.py --knowledge` (M4), `python main.py --diagnostics` (M3), `python main.py --handshake` (M0), or `python -m pytest` (tests). A SessionStart hook (`.claude/`) auto-installs deps + the spaCy model in web sessions.
- **Next action:** Milestone 6 in `ROADMAP.md` — the **Visionary (`0x00D4`)** (simulation/creative, the sixth and final Family member) + **hardening** (optional swap to Redis/NATS bus, SQLite→Postgres, message signing/auth, packaging). Brings the creative SDR types online and completes the six-agent Family.
