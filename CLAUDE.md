...
- **How to run:** `python -m venv .venv && . .venv/bin/activate && pip install -e ".[dev,graph]"` (the `graph` extra pulls spaCy + the `en_core_web_sm` model + networkx; add `web` for the browser UI), then `python main.py` (console — set `ANTHROPIC_API_KEY` for live Claude, else offline mode; commands `/create <brief>`, `/log`, `/health`, `/graph`, `/feedback`, `/policy`, `/quit`), `python main.py --web` (a FastAPI browser interface — `aletheia/web/`, wraps the `QASystem`, shows answers/creative packages + the Domino Cascade; needs the `web` extra), `python main.py --creative` (M6 creative cascade), `--resonance` (M5), `--knowledge` (M4), `--diagnostics` (M3), `--handshake` (M0), or `python -m pytest` (tests). The repo ships a **`.devcontainer/`** so it runs one-click in **GitHub Codespaces**. A SessionStart hook (`.claude/`) auto-installs deps + the spaCy model in web sessions.
